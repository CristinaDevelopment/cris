...
@Resolver(() => Page0)
export class Pages0EcommerceResolver {
  constructor(private readonly pagesService: PagesEcommerceService) {}

  @Mutation(() => Page0, { name: 'createPage0Ecommerce' })
  createPage(@Args('input') input: CreatePage) {
    return this.pagesService.create(input, 'page0');
  }
  @Mutation(() => Page0, { name: 'updatePage0Ecommerce' })
  updatePage(
    @Args() id: GetPage,
  ...
  @Mutation(() => String, { name: 'deletePage0Ecommerce' })
  deletePage(@Args() id: GetPage) {
    return this.pagesService.deletePage(id, 'page0');
  }

  @Mutation(() => [String], { name: 'deletePages0EcommerceById' })
  deletePagesById(
  ...
  @Query(() => Page0, { name: 'findPage0Ecommerce' })
  findPage(@Args() id: GetPage) {
    return this.pagesService.findPage(id, 'page0');
  }

  @Query(() => [Page0], { name: 'findPages0Ecommerce' })
  findPages() {
    return this.pagesService.findPages('page0');
  }

  @Query(() => [Page0], { name: 'findPages0EcommerceByParentId' })
  findPagesByParentId(
  ...
  @Query(() => ListPage, { name: 'listPages0EcommerceWithCursor' })
