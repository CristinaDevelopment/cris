...
import { PagesEducationService } from '../pages.education.service';
...
@Resolver(() => Page1)
export class Pages1EducationResolver {
  constructor(private readonly pagesService: PagesEducationService) {}
  @Mutation(() => Page1, { name: 'createPage1Education' })
  createPage(@Args('input') input: CreatePage) {
    return this.pagesService.create(input, 'page1');
  }
  @Mutation(() => Page1, { name: 'updatePage1Education' })
  updatePage(
    @Args() id: GetPage,
  ...
  @Mutation(() => String, { name: 'deletePage1Education' })
  deletePage(@Args() id: GetPage) {
    return this.pagesService.deletePage(id, 'page1');
  }

  @Mutation(() => [String], { name: 'deletePages1EducationById' })
  deletePagesById(
  ...
  @Query(() => Page1, { name: 'findPage1Education' })
  findPage(@Args() id: GetPage) {
    return this.pagesService.findPage(id, 'page1');
  }

  @Query(() => [Page1], { name: 'findPages1Education' })
  findPages() {
    return this.pagesService.findPages('page1');
  }

  @Query(() => [Page1], { name: 'findPages1EducationByParentId' })
  findPagesByParentId(
  ...
  @Query(() => ListPage, { name: 'listPages1EducationWithCursor' })
