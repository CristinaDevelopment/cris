...
@Resolver(() => Page0)
export class Pages0EducationResolver {
  constructor(private readonly pagesService: PagesEducationService) {}

  @Mutation(() => Page0, { name: 'createPage0Education' })
  createPage(@Args('input') input: CreatePage) {
    return this.pagesService.create(input, 'page0');
  }
  @Mutation(() => Page0, { name: 'updatePage0Education' })
  updatePage(
    @Args() id: GetPage,
  ...
  @Mutation(() => String, { name: 'deletePage0Education' })
  deletePage(@Args() id: GetPage) {
    return this.pagesService.deletePage(id, 'page0');
  }

  @Mutation(() => [String], { name: 'deletePages0EducationById' })
  deletePagesById(
  ...
  @Query(() => Page0, { name: 'findPage0Education' })
  findPage(@Args() id: GetPage) {
    return this.pagesService.findPage(id, 'page0');
  }

  @Query(() => [Page0], { name: 'findPages0Education' })
  findPages() {
    return this.pagesService.findPages('page0');
  }

  @Query(() => [Page0], { name: 'findPages0EducationByParentId' })
  findPagesByParentId(
  ...
  @Query(() => ListPage, { name: 'listPages0EducationWithCursor' })
